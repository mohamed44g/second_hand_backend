...
export const getAllBids = async () => {
  const result = await pool.query(
    `SELECT b.*, m.main_category_name, u.username AS bidder_username, d.*,
      CASE 
...
export const finalizeAuction = async (bid_id) => {
  ...
  try {
    ...
    await client.query(
      `UPDATE Bids SET is_ended = TRUE, SET winning_bid_id  = $2 WHERE bid_id = $1`,
      [bid_id, winnerId]
    );
...
export const cancelBidSellerDb = async (bid_id) => {
  ...
  if (bid.minimumnoncancellableprice <= getHighestBid.bid_amount) {
    throw new AppError(
  ...
  const result = await pool.query(
    `UPDATE Bids SET is_ended = TRUE, SET status = 'cancled' WHERE bid_id = $1`,
    [bid_id]
  );
  return result.rows[0];
};
